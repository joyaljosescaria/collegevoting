...
function Liveresult(props) {
    ...
    useEffect(() => {
        function mainf() {
            props.getOld()
            if (props.publics.candidate1) {
                data.length = 0
                for (let i = 0; i < props.publics.candidate1.length; i++) {
                    props.publics.candidate1[i].map((pos, index) => {
                        var obj = {};
...
export default connect(mapStateToProps, { getOld })(Liveresult)
